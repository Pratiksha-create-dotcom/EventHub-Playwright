...
async function createBookingFromFilters(page, bookingData) {
    await page.getByPlaceholder("Search events, venues…").fill(bookingData.search);
    ...
    const eventCard = page.getByTestId("event-card").filter({ hasText: bookingData.search });
    await eventCard.getByTestId("book-now-btn").click();
    let currentTickets = Number(await page.locator("#ticket-count").textContent());
    ...
    const ticketCount = Number(
     await page.locator("#ticket-count").textContent());
     await page.getByPlaceholder("Your full name").fill(bookingData.name); 
    const customerEmail = bookingData.email;
    await page.getByTestId("customer-email").fill(customerEmail); 
    await page.locator("#phone").fill(bookingData.phone);
    ...
    //const totalText = (await page.locator("div:has-text('Total') span.font-medium").textContent()).trim();
   const totalText = await page.locator("div:has-text('Total') span.font-medium").nth(3).textContent();
    return {
        eventTitle,
        bookingRef,
        ticketCount,
...
async function findBookingCardByRef(page, bookingRef) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const ref = (
            await card.locator(".booking-ref").textContent()
        ).trim();
...
test("Verify booking details", async ({ page }) => {
     await login(page, email, password);
     const booking = await createBookingFromFilters(page, bookingData);
     await page.goto("/bookings");
     const bookingCard = await findBookingCardByRef(
        page,
        booking.bookingRef
    );
...
test.only("End-to-End Workflows", async ({ page }) => {
    //Navigate to /login
...
const bookingId = (await bookingInformation.locator("span.text-sm.font-medium.text-gray-900.text-right").last().textContent()).trim();
